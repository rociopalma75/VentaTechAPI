...
namespace Application.Validators
{
    public class BaseValidator<TRequestDTO> : AbstractValidator<TRequestDTO> where TRequestDTO : class
    {
        ...
        protected void ValidateDateTime(Expression<Func<TRequestDTO, DateTime>> propertyExpression)
        {
            RuleFor(propertyExpression)
                .NotEmpty().WithMessage("{PropertyName} es requerido")
                .LessThanOrEqualTo(DateTime.Now).WithMessage("{PropertyName} no puede ser posterior al dia de hoy");
        }
